...
import "./tich-hop-gis.style.scss";
const FileType = {
    GDB: "GDB",
    GEOJSON: "GeoJson",
    SHAPEFILE: "ShapeFile",
...
class TichHopGISView implements IBaseComponent {
    ...
    constructor(container: JQuery<HTMLElement>, oGConfig: OGConfigModel) {
        this.layerViewsContainer = container;
                    ...
                    $("<div>").appendTo(container).dxToolbar({
                        items: [{
                            ...
                            options: {
                                ...
                                icon: "icon icon-data",
                                onClick: () => {
                                    this.attributesWindowComponent.for(options.data, undefined).show();
                                },
                                type: "success"
                            },
                            widget: "dxButton"
                            ...
                            options: {
                                ...
                                onClick: () => {
                                    ...
                                    self.layerGrid.editRow(options.rowIndex);
                                },
                                type: "success"
                            },
                            widget: "dxButton"
                                                    ...
                                                    if (_) {
                                                        axios.post("/api/layer/" + self.selectedLayerInfo.id + "/clear-data").then(result => {
                                                            if (result.data.status === EnumStatus.OK) {
                                                                OGUtils.alert("Xóa dữ liệu lớp thành công!");
                                                            } else {
                                                                OGUtils.error(result.data.errors[0].message);
                                                            }
                                                        });
                                                    }
                                                });
                                                    ...
                                                    if (validate && validate.brokenRules.length === 0) {
                                                        ...
                                                        axios({
                                                            ...
                                                            if (result.data && result.data.status === EnumStatus.OK) {
                                                                OGUtils.alert("Sao chép lớp dữ liệu thành công!");
                                                                this.layerGrid.getDataSource().reload();
                                                        ...
                                                        if (self.importFiles && self.importFiles.length) {
                                                            ...
                                                            xhr.onloadend = () => {
                                                                ...
                                                                if (xhr.response) {
                                                                    this.layerGrid.getDataSource().reload();
                                                                    if (xhr.response.status == "OK") {
                                                                        OGUtils.alert("Cập nhật dữ liệu schema liệu thành công");
