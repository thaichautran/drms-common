...
class CategoryView implements IBaseComponent {
    categoryForm: dxForm;
                        ...
                        load: () => {
                            ...
                            axios("/api/category/trees").then(xhr => {
                                if (xhr.data.status === EnumStatus.OK) {
                                    def.resolve({
                                        data: xhr.data.data.filter(x => x.text === "Danh mục gói thầu dự án"),
                                        totalCount: xhr.data.data.filter(x => x.text === "Danh mục gói thầu dự án").length
                                    });
                                } else {
