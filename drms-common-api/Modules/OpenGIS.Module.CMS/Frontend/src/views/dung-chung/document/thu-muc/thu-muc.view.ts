...
class ThuMucView implements IBaseComponent {
    attachmentForm: dxForm;
                            ...
                            items: [
                                {
                                    location: "before",
                                    template: () => {
                                        return "<h6>QUẢN LÝ TỆP ĐÍNH KÈM</h6>";
                                    }
                                },
                                {
                                    location: "after",
    ...
    private initLayout(): void {
        ...
        this.attachmentPopup = $("<div />").appendTo("body").dxPopup({
            contentTemplate: (container) => {
                this.attachmentForm = $("<form />").appendTo(container)
                    .dxForm({
                        formData: {
                            file_name: "",
                        },
                                ...
                                this.dxAttachmentUpload = $(itemElement)
                                    .dxFileUploader({
                                        ...
                                        onValueChanged: function (e) {
                                            ...
                                            if (files.length > 0) {
                                                const file = files[0];
                                                self.attachmentForm.option("formData", { file_name: file.name });
                                            }
                                        },
                                ...
                                $("<div />").appendTo(itemElement)
                                    .dxToolbar({
                                        items: [{
                                            options: {
                                                onClick: () => {
                                                    const validate = this.attachmentForm.validate();
                                                    if (validate && validate.brokenRules.length === 0) {
                                                        const data = this.attachmentForm.option("formData");
