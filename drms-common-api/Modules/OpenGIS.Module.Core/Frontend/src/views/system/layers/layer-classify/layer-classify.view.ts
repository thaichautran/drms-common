...
class LayerClassifyView implements IBaseComponent {
    ...
    private fieldSelector: dxSelectBox;
    private layerClassifyFieldSelectBox: dxSelectBox;
    private layerClassifyGrid: dxDataGrid;
    private layerClassifyStore: CustomStore<OGLayerClassifyModel, number>;
    private layerFieldStore: CustomStore<OGTableColumnModel, number>;
    private layerViews: dxMultiView;
    private oGLayer: OGLayerModel;
    private styleEditor: StyleEditorComponent;
    ...
    private initLayerClassifyGrid(container): void {
        this.layerClassifyGrid = $("<div />").appendTo(container).dxDataGrid({
            ...
            columns: [{
                ...
                caption: "Thao tác",
                cellTemplate: (container, options) => {
                    const data = options.data as OGLayerClassifyModel;
                    //
                        ...
                        items: [{
                            ...
                            options: {
                                ...
                                onClick: () => {
                                    ...
                                    // options.component.getDataSource().store().update(options.value).then(() => {
                            ...
                            options: {
                                ...
                                onClick: () => {
                                    OGUtils.confirm("Xác nhận xóa classify này?").then(value => {
                                        if (value) {
                            ...
                            options: {
                                onClick: () => {
                                    ...
                                    const def = $.Deferred();
                                    this.styleEditor.forStyle(this.oGLayer, JSON.parse(style), def).show();
                                    def.then((e) => {
                                        if (e.type === "style") {
                                            $.post("/api/layer/classify/" + options.data.id + "/set-style", {
                ...
                e.toolbarOptions.items.unshift({
                    ...
                    options: {
                        ...
                        onSelectionChanged: (e) => {
                            if (e.selectedItem) {
                                $.get("/api/layer/" + this.oGLayer.id + "/list-classify-values", {
                    ...
                    options: {
                        onClick: () => {
                            ...
                            if (selectedItem) {
                                OGUtils.confirm("Xác nhận tạo lại toàn bộ classify? Các classify đã tạo sẽ mất!").then(_ => {
                                    if (_) {
                                        $.post("/api/layer/" + this.oGLayer.id + "/initial-classify-values", {
                    ...
                    options: {
                        onClick: () => {
                            const selectedItem = this.layerClassifyFieldSelectBox.option("selectedItem");
                            if (selectedItem) {
                                OGUtils.confirm("Xác nhận xóa toàn bộ classify?").then(_ => {
                                    if (_) {
                                        $.post("/api/layer/" + this.oGLayer.id + "/delete-classify-values", {
                                            column_id: selectedItem.id
                                        }).done(() => {
                                            this.layerClassifyGrid.refresh(true);
                                        });
                    ...
                    options: {
                        onClick: () => {
                            this.layerClassifyGrid.getDataSource().items().forEach(async (item: OGLayerClassifyModel, idx) => {
                                let style;
                                ...
                                if (style) {
                                    $.post("/api/layer/classify/" + item.id + "/set-style", {
                                        style: style
                                    }).then(() => {
                                        this.layerClassifyGrid.refresh(true);
                                    });
        ...
        this.layerClassifyStore = new CustomStore({
            ...
            insert: (values) => {
                ...
                return new Promise((resolve, reject) => {
                    values.layer_id = self.oGLayer.id;
                    values.table_column_id = self.layerClassifyFieldSelectBox.option("value");
                    axios.post("/api/layer/classify", values).then(xhr => {
                        if (xhr.data && xhr.data.status === EnumStatus.OK) {
            ...
            load: (loadOptions) => {
                ...
                if (this.oGLayer && this.oGLayer.id && this.layerClassifyFieldSelectBox && this.layerClassifyFieldSelectBox.option("value")) {
                    $.get("/api/layer/" + this.oGLayer.id + "/list-classify-values", {
            ...
            remove: (key) => {
                return new Promise((resolve, reject) => {
                    if (key) {
                        axios.delete("/api/layer/classify/" + key.toString()).then(xhr => {
                            if (xhr.data && xhr.data.status === EnumStatus.OK) {
                                resolve();
                            } else {
                                reject();
            ...
            update: (key, values) => {
                return new Promise((resolve, reject) => {
                    if (key) {
                        axios.put("/api/layer/classify", values).then(xhr => {
                            if (xhr.data && xhr.data.status === EnumStatus.OK) {
                                resolve(xhr.data.data);
                            } else {
                                reject();
