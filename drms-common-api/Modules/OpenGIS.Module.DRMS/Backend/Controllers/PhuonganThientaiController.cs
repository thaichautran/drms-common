...
namespace OpenGIS.Module.DRMS.Areas.DRMS.Controllers
{
    ...
    [Route("api/phuongan-thientai")]
    public class PhuonganThientaiController : BaseApiCRUDController<INpgsqlSession, PhuonganThientai, int>
    {
        public PhuonganThientaiController(IDbFactory dbFactory, IMapper mapper, IRepository<PhuonganThientai, int> repository)
        ...
        public async Task<RestBase> DatatableAsync([FromBody] DataTableParameters dataTb)
        {
            ...
            string tableAlias = typeof(PhuonganThientai).Name.ToLower();
            string orderName = $" 1 ASC";
            string orderDir = "ASC";
            List<PhuonganThientai> data;

            var recordsTotal = await session.CountAsync<PhuonganThientai>(statement => statement
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.FindAsync<PhuonganThientai>(statement => statement.WithAlias(tableAlias).Where($"{condition}")
            ...
            {
                data = new List<PhuonganThientai>();
            }
            else
            {
                data = (await session.FindAsync<PhuonganThientai>(statement => statement
                    .WithAlias(tableAlias).Where($"{condition}")
                    .WithParameters(withParams)
            ...
            return new RestPagedDataTable<IEnumerable<PhuonganThientai>>
            {
                data = data,
                recordsFiltered = await session.CountAsync<PhuonganThientai>(statement => statement
