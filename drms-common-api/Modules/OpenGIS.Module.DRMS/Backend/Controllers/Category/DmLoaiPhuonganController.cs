...
namespace OpenGIS.Module.DRMS.Areas.DRMS.Controllers
{
    ...
    [Route("api/dm-loai-phuongan")]
    public class DmLoaiPhuonganController : BaseApiCRUDController<INpgsqlSession, DmLoaiPhuongan, int>
    {
        public DmLoaiPhuonganController(IDbFactory dbFactory, IMapper mapper, IRepository<DmLoaiPhuongan, int> repository)
        ...
        public async Task<RestBase> DatatableAsync([FromBody] DataTableParameters dataTb)
        {
            ...
            string tableAlias = typeof(DmLoaiPhuongan).Name.ToLower();
            string orderName = $" 1 ASC";
            string orderDir = "ASC";
            List<DmLoaiPhuongan> data;

            var recordsTotal = await session.CountAsync<DmLoaiPhuongan>(statement => statement
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.FindAsync<DmLoaiPhuongan>(statement => statement.WithAlias(tableAlias).Where($"{condition}")
            ...
            {
                data = new List<DmLoaiPhuongan>();
            }
            else
            {
                data = (await session.FindAsync<DmLoaiPhuongan>(statement => statement
                    .WithAlias(tableAlias).Where($"{condition}")
                    .WithParameters(withParams)
            ...
            return new RestPagedDataTable<IEnumerable<DmLoaiPhuongan>>
            {
                data = data,
                recordsFiltered = await session.CountAsync<DmLoaiPhuongan>(statement => statement
