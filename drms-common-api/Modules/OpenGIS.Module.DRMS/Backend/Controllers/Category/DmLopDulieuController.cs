...
namespace OpenGIS.Module.DRMS.Areas.DRMS.Controllers
{
    ...
    [Route("api/dm-lop-dulieu")]
    public class DmLopDulieuController : BaseApiCRUDController<INpgsqlSession, DmLopDulieu, int>
    {
        public DmLopDulieuController(IDbFactory dbFactory, IMapper mapper, IRepository<DmLopDulieu, int> repository)
        ...
        public async Task<RestBase> DatatableAsync([FromBody] DataTableParameters dataTb)
        {
            ...
            string tableAlias = typeof(DmLopDulieu).Name.ToLower();
            string orderName = $" 1 ASC";
            string orderDir = "ASC";
            List<DmLopDulieu> data;

            var recordsTotal = await session.CountAsync<DmLopDulieu>(statement => statement
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.FindAsync<DmLopDulieu>(statement => statement.WithAlias(tableAlias).Where($"{condition}")
            ...
            {
                data = new List<DmLopDulieu>();
            }
            else
            {
                data = (await session.FindAsync<DmLopDulieu>(statement => statement
                    .WithAlias(tableAlias).Where($"{condition}")
                    .WithParameters(withParams)
            ...
            return new RestPagedDataTable<IEnumerable<DmLopDulieu>>
            {
                data = data,
                recordsFiltered = await session.CountAsync<DmLopDulieu>(statement => statement
