...
namespace OpenGIS.Module.DRMS.Areas.DRMS.Controllers
{
    ...
    [Route("api/dm-loaibando")]
    public class DmLoaiBanDoController : BaseApiCRUDController<INpgsqlSession, DmLoaiBanDo, int>
    {
        public DmLoaiBanDoController(IDbFactory dbFactory, IMapper mapper, IRepository<DmLoaiBanDo, int> repository)
        ...
        public async Task<RestBase> DatatableAsync([FromBody] DataTableParameters dataTb)
        {
            ...
            string tableAlias = typeof(DmLoaiBanDo).Name.ToLower();
            string orderName = $" 1 ASC";
            string orderDir = "ASC";
            List<DmLoaiBanDo> data;

            var recordsTotal = await session.CountAsync<DmLoaiBanDo>(statement => statement
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.FindAsync<DmLoaiBanDo>(statement => statement.WithAlias(tableAlias).Where($"{condition}")
            ...
            {
                data = new List<DmLoaiBanDo>();
            }
            else
            {
                data = (await session.FindAsync<DmLoaiBanDo>(statement => statement
                    .WithAlias(tableAlias).Where($"{condition}")
                    .WithParameters(withParams)
            ...
            return new RestPagedDataTable<IEnumerable<DmLoaiBanDo>>
            {
                data = data,
                recordsFiltered = await session.CountAsync<DmLoaiBanDo>(statement => statement
