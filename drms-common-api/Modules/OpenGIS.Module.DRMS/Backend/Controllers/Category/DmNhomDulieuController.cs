...
namespace OpenGIS.Module.DRMS.Areas.DRMS.Controllers
{
    ...
    [Route("api/dm-nhom-dulieu")]
    public class DmNhomDulieuController : BaseApiCRUDController<INpgsqlSession, DmNhomDulieu, int>
    {
        public DmNhomDulieuController(IDbFactory dbFactory, IMapper mapper, IRepository<DmNhomDulieu, int> repository)
        ...
        public async Task<RestBase> DatatableAsync([FromBody] DataTableParameters dataTb)
        {
            ...
            string tableAlias = typeof(DmNhomDulieu).Name.ToLower();
            string orderName = $" 1 ASC";
            string orderDir = "ASC";
            List<DmNhomDulieu> data;

            var recordsTotal = await session.CountAsync<DmNhomDulieu>(statement => statement
            ...
            if (dataTb?.length == -1)
            {
                data = (await session.FindAsync<DmNhomDulieu>(statement => statement.WithAlias(tableAlias).Where($"{condition}")
            ...
            {
                data = new List<DmNhomDulieu>();
            }
            else
            {
                data = (await session.FindAsync<DmNhomDulieu>(statement => statement
                    .WithAlias(tableAlias).Where($"{condition}")
                    .WithParameters(withParams)
            ...
            return new RestPagedDataTable<IEnumerable<DmNhomDulieu>>
            {
                data = data,
                recordsFiltered = await session.CountAsync<DmNhomDulieu>(statement => statement
