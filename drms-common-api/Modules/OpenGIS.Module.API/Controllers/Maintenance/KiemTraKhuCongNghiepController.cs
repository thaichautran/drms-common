...
using OpenGIS.Module.Core.Models.Entities.Maintenance.KhuCongNghiep;
...
{
    [Route("api/khu-cong-nghiep/kiem-tra")]
    //// [Authorize(Policy = nameof(ModuleFunction._APPROVE))]
    public class KiemTraKhuCongNghiepController : BaseController
    {
        private readonly IWebHostEnvironment _webHostEnvironment;
        private readonly IEmailSender _emailSender;
        private readonly IWorkContext _workContext;
        public KiemTraKhuCongNghiepController(
        ...
        [HttpPost("notify")]
        public async Task<RestBase> NotifyAsync([FromQuery] bool isCreated, [FromBody] PhieuGiamSatKiemTraKhuCongNghiep item, [FromQuery] bool sendMail = false)
        {
        ...
        [HttpPost("list")]
        public async Task<RestBase> List([FromForm] KiemTraKhuCongNghiepDxDTO? dto)
        {
            if (dto == null)
            {
            ...
            using (var session = OpenSession())
            {
                List<PhieuGiamSatKiemTraKhuCongNghiep> data = new List<PhieuGiamSatKiemTraKhuCongNghiep>();
                string condition = "1=1";
                if (dto.phuongthuckiemtraid.HasValue && dto.phuongthuckiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.phuongthuckiemtraid):TC} = @phuongthuckiemtraid";
                }
                if (dto.congcukiemtraid.HasValue && dto.congcukiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.congcukiemtraid):TC} = @congcukiemtraid";
                }
                if (dto.trangthaicongviec.HasValue && dto.trangthaicongviec.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.trangthaicongviec):TC} = @trangthaicongviec";
                }
                if (dto.ngaythuchien.HasValue)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.ngaythuchien):TC} = @ngaythuchien";
                }
                if (!string.IsNullOrWhiteSpace(dto?.loaicongviec))
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.loaicongviec):TC} = @loaicongviec";
                }
                if (!string.IsNullOrWhiteSpace(dto?.phongban))
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.id):TC} In (SELECT {Sql.Entity<GiaoViecNhanVien>(x => x.phieugiamsat_id):TC} FROM {Sql.Entity<GiaoViecNhanVien>():T} LEFT JOIN {Sql.Entity<NhanVien>():T} ON {Sql.Entity<GiaoViecNhanVien>(x => x.nhanvien_id):TC} = {Sql.Entity<NhanVien>(x => x.id):TC} WHERE {Sql.Entity<NhanVien>(x => x.donvicongtac):TC} = @phongban)";
                }
                if (dto.isCompleted.HasValue && dto.isCompleted.Value == false)
                {
                    dto.thoigian = DateTime.Now.Date;
                    condition += $" AND DATE({Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.ngaythuchien):TC}) >= @thoigian";
                }
                if (dto.take == 0)
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.ngaythuchien):TC} DESC")
                    )).ToList();
                }
                else
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm.Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.ngaythuchien):TC} DESC")
                ...
                {
                    data = data,
                    recordsTotal = await session.CountAsync<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto))
                };
            }
        ...
        {
            using var session = OpenSession();
            var condition = $"{Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.loaicongviec):TC} <> ''";
            return new RestData
            {
                data = session.Query<string>($"SELECT DISTINCT {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.loaicongviec):TC} FROM {Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>():T} WHERE {condition}")
            };
        }
        [HttpPost("save")]
        //// [Authorize(Policy = nameof(ModuleFunction.EDIT_WORKER))]
        public async Task<RestBase> save([FromBody] PhieuGiamSatKiemTraKhuCongNghiep item)
        {
            using (var session = OpenSession())
            {
                if (item == null)
                ...
                else
                {
                    ...
                    {
                        var existItem = session.Get(new PhieuGiamSatKiemTraKhuCongNghiep { id = item.id });
                        if (existItem == null)
                        {
                            return new RestError
                            {
                    ...
                    if (item.congTrinhBaoDuongs != null)
                    {
                        if (item.congTrinhBaoDuongs.Count() > 0)
                        {
                            foreach (KiemTraBaoDuongCongTrinh congTrinh in item.congTrinhBaoDuongs)
                            {
                                ...
                                congTrinh.loaikiemtra = EnumLoaiKiemTra.KHUCONGNGHIEP;
                    ...
                    if (item.giaoViecNhanViens != null && item.giaoViecNhanViens.Count() > 0)
                    {
                        foreach (GiaoViecNhanVien giaoViecNhanVien in item.giaoViecNhanViens)
                        {
                            ...
                            giaoViecNhanVien.loaikiemtra = EnumLoaiKiemTra.KHUCONGNGHIEP;
                        ...
                        if (item.thongTinTraoDois.Count() > 0)
                        {
                            foreach (ThongTinTraoDoiKiemTra thongTinTraoDoi in item.thongTinTraoDois)
                            {
                                if (thongTinTraoDoi.id == 0)
                                {
                                    ...
                                    thongTinTraoDoi.loaikiemtra = EnumLoaiKiemTra.KHUCONGNGHIEP;
        ...
        public RestBase Get([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                PhieuGiamSatKiemTraKhuCongNghiep? data = session.Find<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm
                    .Where($"{Sql.Entity<PhieuGiamSatKiemTraKhuCongNghiep>(x => x.id):TC} = @id")
                ...
                if (data != null)
                {
                    ...
                    data.thongTinTraoDois = session.Find<ThongTinTraoDoiKiemTra>(stm => stm
                        .Where(@$"{Sql.Entity<ThongTinTraoDoiKiemTra>(x => x.phieugiamsat_id):TC} = @id 
                            AND {Sql.Entity<ThongTinTraoDoiKiemTra>(x => x.loaikiemtra):TC} = '{EnumLoaiKiemTra.KHUCONGNGHIEP}'")
                        .WithParameters(data)
                    ).ToList();
                }
                return new RestData
        ...
        public RestBase Delete([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                var existItem = session.Get(new PhieuGiamSatKiemTraKhuCongNghiep { id = id });
                if (existItem == null)
                {
                    return new RestError
                    {
                ...
                else
                {
                    session.Execute(@$"DELETE FROM {Sql.Entity<HoSoQuanLyKiemTra>():T} 
                            WHERE {nameof(HoSoQuanLyKiemTra.phieugiamsat_id)} = @id AND {nameof(HoSoQuanLyKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.KHUCONGNGHIEP}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<AnhMinhHoaKiemTra>():T} 
                            WHERE {nameof(AnhMinhHoaKiemTra.phieugiamsat_id)} = @id AND {nameof(AnhMinhHoaKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.KHUCONGNGHIEP}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<GiaoViecNhanVien>():T} 
                            WHERE {nameof(GiaoViecNhanVien.phieugiamsat_id)} = @id AND {nameof(GiaoViecNhanVien.loaikiemtra):TC} = '{EnumLoaiKiemTra.KHUCONGNGHIEP}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<ThongTinTraoDoiKiemTra>():T} 
                            WHERE {nameof(ThongTinTraoDoiKiemTra.phieugiamsat_id)} = @id AND {nameof(ThongTinTraoDoiKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.KHUCONGNGHIEP}'", existItem);
        ...
        [HttpPost("export")]
        public async Task<IActionResult> Export([FromForm] KiemTraKhuCongNghiepDxDTO dto)
        {
            using (var session = OpenSession())
            {
                string condition = "1=1";
                ...
                List<PhieuGiamSatKiemTraKhuCongNghiep> data = (await session.FindAsync<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm
                    .Where($"{condition}")
                    .WithParameters(dto)
                    .Include<PhuongThucKiemTra>(x => x.LeftOuterJoin())
                    .Include<CongCuKiemTra>(x => x.LeftOuterJoin())
                ...
                int totalCount = await session.CountAsync<PhieuGiamSatKiemTraKhuCongNghiep>(stm => stm
                           .Where($"{condition}")
                           .WithParameters(dto));

                ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
                string fileName = "BaoCaoKiemTraKhuCongNghiep.xlsx";
