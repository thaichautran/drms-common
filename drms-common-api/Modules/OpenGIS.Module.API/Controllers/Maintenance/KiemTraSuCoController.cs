...
using OpenGIS.Module.Core.Models.Entities.Maintenance.SuCo;
...
{
    [Route("api/su-co/kiem-tra")]
    //// [Authorize(Policy = nameof(ModuleFunction._APPROVE))]
    public class KiemTraSuCoController : BaseController
    {
        private readonly IWebHostEnvironment _webHostEnvironment;
        private readonly IEmailSender _emailSender;
        private readonly IWorkContext _workContext;
        public KiemTraSuCoController(
        ...
        [HttpPost("notify")]
        public async Task<RestBase> NotifyAsync([FromQuery] bool isCreated, [FromBody] PhieuGiamSatKiemTraSuCo item)
        {
        ...
        public async Task<RestBase> List([FromForm] KiemTraListDxDTO? dto)
        {
            ...
            using (var session = OpenSession())
            {
                List<PhieuGiamSatKiemTraSuCo> data = new List<PhieuGiamSatKiemTraSuCo>();
                string condition = "1=1";
                if (dto.phuongthuckiemtraid.HasValue && dto.phuongthuckiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.phuongthuckiemtraid):TC} = @phuongthuckiemtraid";
                }
                if (dto.congcukiemtraid.HasValue && dto.congcukiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.congcukiemtraid):TC} = @congcukiemtraid";
                }
                if (dto.isCompleted.HasValue && dto.isCompleted.Value == false)
                {
                    dto.thoigian = DateTime.Now.Date;
                    condition += $" AND DATE({Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.ngaythuchien):TC}) >= @thoigian";
                }
                if (dto.take == 0)
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraSuCo>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.ngaythuchien):TC} DESC")
                    )).ToList();
                }
                else
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraSuCo>(stm => stm.Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.ngaythuchien):TC} DESC")
                ...
                {
                    data = data,
                    recordsTotal = await session.CountAsync<PhieuGiamSatKiemTraSuCo>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto))
                };
            }
        ...
        public async Task<RestBase> save([FromBody] PhieuGiamSatKiemTraSuCo item)
        {
            using (var session = OpenSession())
            {
                if (item == null)
                ...
                else
                {
                    ...
                    {
                        var existItem = session.Get(new PhieuGiamSatKiemTraSuCo { id = item.id });
                        if (existItem == null)
                        {
                            return new RestError
                            {
                    ...
                    if (item.congTrinhBaoDuongs != null)
                    {
                        if (item.congTrinhBaoDuongs.Count() > 0)
                        {
                            foreach (KiemTraBaoDuongCongTrinh congTrinh in item.congTrinhBaoDuongs)
                            {
                                ...
                                congTrinh.loaikiemtra = EnumLoaiKiemTra.VITRI_SUCO;
                    ...
                    if (item.giaoViecNhanViens != null && item.giaoViecNhanViens.Count() > 0)
                    {
                        foreach (GiaoViecNhanVien giaoViecNhanVien in item.giaoViecNhanViens)
                        {
                            ...
                            giaoViecNhanVien.loaikiemtra = EnumLoaiKiemTra.VITRI_SUCO;
                    ...
                    {
                        await session.ExecuteAsync($"DELETE FROM {Sql.Entity<HoSoQuanLyKiemTra>():T} WHERE {nameof(HoSoQuanLyKiemTra.id)} = ANY(@hoso_ids)", new { hoso_ids = item.deleteHoSoQuanLyIds.ToArray() });
                    }
                        ...
                        if (item.thongTinTraoDois.Count() > 0)
                        {
                            foreach (ThongTinTraoDoiKiemTra thongTinTraoDoi in item.thongTinTraoDois)
                            {
                                if (thongTinTraoDoi.id == 0)
                                {
                                    ...
                                    thongTinTraoDoi.loaikiemtra = EnumLoaiKiemTra.VITRI_SUCO;
        ...
        public RestBase Get([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                PhieuGiamSatKiemTraSuCo? data = session.Find<PhieuGiamSatKiemTraSuCo>(stm => stm
                    .Where($"{Sql.Entity<PhieuGiamSatKiemTraSuCo>(x => x.id):TC} = @id")
                ...
                if (data != null)
                {
                    ...
                    data.thongTinTraoDois = session.Find<ThongTinTraoDoiKiemTra>(stm => stm
                       ...
                       AND {Sql.Entity<ThongTinTraoDoiKiemTra>(x => x.loaikiemtra):TC} = '{EnumLoaiKiemTra.VITRI_SUCO}'")
                        .WithParameters(data)
                    ).ToList();
                }
                return new RestData
        ...
        public RestBase Delete([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                var existItem = session.Get(new PhieuGiamSatKiemTraSuCo { id = id });
                if (existItem == null)
                {
                    return new RestError
                    {
                ...
                else
                {
                    session.Execute(@$"DELETE FROM {Sql.Entity<HoSoQuanLyKiemTra>():T} 
                            WHERE {nameof(HoSoQuanLyKiemTra.phieugiamsat_id)} = @id AND {nameof(HoSoQuanLyKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.VITRI_SUCO}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<AnhMinhHoaKiemTra>():T} 
                            WHERE {nameof(AnhMinhHoaKiemTra.phieugiamsat_id)} = @id AND {nameof(AnhMinhHoaKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.VITRI_SUCO}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<GiaoViecNhanVien>():T} 
                            WHERE {nameof(GiaoViecNhanVien.phieugiamsat_id)} = @id AND {nameof(GiaoViecNhanVien.loaikiemtra):TC} = '{EnumLoaiKiemTra.VITRI_SUCO}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<ThongTinTraoDoiKiemTra>():T} 
                            WHERE {nameof(ThongTinTraoDoiKiemTra.phieugiamsat_id)} = @id AND {nameof(ThongTinTraoDoiKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.VITRI_SUCO}'", existItem);
        ...
        public async Task<IActionResult> Export([FromForm] KiemTraListDxDTO dto)
        {
            using (var session = OpenSession())
            {
                ...
                List<PhieuGiamSatKiemTraSuCo> data = (await session.FindAsync<PhieuGiamSatKiemTraSuCo>(stm => stm
                    .Where($"{condition}")
                    .WithParameters(dto)
                    .Include<PhuongThucKiemTra>(x => x.LeftOuterJoin())
                    .Include<CongCuKiemTra>(x => x.LeftOuterJoin())
                ...
                int totalCount = await session.CountAsync<PhieuGiamSatKiemTraSuCo>(stm => stm
                            .Where($"{condition}")
                            .WithParameters(dto));

                ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
                string fileName = "BaoCaoKiemTraSuCo.xlsx";
