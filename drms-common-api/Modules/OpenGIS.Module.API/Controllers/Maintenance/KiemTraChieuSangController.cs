...
{
    [Route("api/chieu-sang/kiem-tra")]
    //// [Authorize(Policy = nameof(ModuleFunction._APPROVE))]
    public class KiemTraChieuSangController : BaseController
    {
        private readonly IWebHostEnvironment _webHostEnvironment;
        private readonly IEmailSender _emailSender;
        private readonly IWorkContext _workContext;
        public KiemTraChieuSangController(
        ...
        public async Task<RestBase> List([FromForm] KiemTraListDxDTO? dto)
        {
            ...
            {
                List<PhieuGiamSatKiemTraChieuSang> data = new List<PhieuGiamSatKiemTraChieuSang>();
                string condition = "1=1";
                if (dto.phuongthuckiemtraid.HasValue && dto.phuongthuckiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.phuongthuckiemtraid):TC} = @phuongthuckiemtraid";
                }
                if (dto.congcukiemtraid.HasValue && dto.congcukiemtraid.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.congcukiemtraid):TC} = @congcukiemtraid";
                }
                if (dto.table_id.HasValue && dto.table_id.Value > 0)
                {
                    condition += $" AND {Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.id):TC} IN (SELECT {Sql.Entity<KiemTraBaoDuongCongTrinh>(x => x.phieugiamsat_id):TC} FROM {Sql.Entity<KiemTraBaoDuongCongTrinh>():T} WHERE {Sql.Entity<KiemTraBaoDuongCongTrinh>(x => x.table_id):TC} = @table_id)";
                }
                if (dto.isCompleted.HasValue && dto.isCompleted.Value == false)
                {
                    dto.thoigian = DateTime.Now.Date;
                    condition += $" AND DATE({Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.ngaythuchien):TC}) >= @thoigian";
                }
                if (dto.take == 0)
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraChieuSang>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.ngaythuchien):TC} DESC")
                    )).ToList();
                }
                else
                {
                    data = (await session.FindAsync<PhieuGiamSatKiemTraChieuSang>(stm => stm.Where($"{condition}")
                        .WithParameters(dto)
                        .OrderBy($"{Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.ngaythuchien):TC} DESC")
                ...
                {
                    data = data,
                    recordsTotal = await session.CountAsync<PhieuGiamSatKiemTraChieuSang>(stm => stm
                        .Where($"{condition}")
                        .WithParameters(dto))
                };
            }
        ...
        [HttpPost("notify")]
        public async Task<RestBase> NotifyAsync([FromQuery] bool isCreated, [FromBody] PhieuGiamSatKiemTraChieuSang item)
        {
            using (var session = OpenSession())
            {
        ...
        public async Task<RestBase> save([FromBody] PhieuGiamSatKiemTraChieuSang item)
        {
            using (var session = OpenSession())
            {
                if (item == null)
                ...
                else
                {
                    ...
                    {
                        var existItem = session.Get(new PhieuGiamSatKiemTraChieuSang { id = item.id });
                        if (existItem == null)
                        {
                            return new RestError
                            {
                    ...
                    session.Execute($"DELETE FROM {Sql.Entity<KiemTraBaoDuongCongTrinh>():T} WHERE {nameof(KiemTraBaoDuongCongTrinh.phieugiamsat_id)} = @id AND {nameof(KiemTraBaoDuongCongTrinh.loaikiemtra)} = '{EnumLoaiKiemTra.CHIEUSANG}'", item);
                    if (item.congTrinhBaoDuongs != null)
                    {
                        if (item.congTrinhBaoDuongs.Count() > 0)
                        {
                            foreach (KiemTraBaoDuongCongTrinh congTrinh in item.congTrinhBaoDuongs)
                            {
                                ...
                                congTrinh.loaikiemtra = EnumLoaiKiemTra.CHIEUSANG;
                                await session.InsertAsync(congTrinh);
                            }
                        }
                    }

                    session.Execute($"DELETE FROM {Sql.Entity<GiaoViecNhanVien>():T} WHERE {nameof(GiaoViecNhanVien.phieugiamsat_id)} = @id AND {nameof(GiaoViecNhanVien.loaikiemtra)} = '{EnumLoaiKiemTra.CHIEUSANG}'", item);
                        ...
                        if (item.thongTinTraoDois.Count() > 0)
                        {
                            foreach (ThongTinTraoDoiKiemTra thongTinTraoDoi in item.thongTinTraoDois)
                            {
                                if (thongTinTraoDoi.id == 0)
                                {
                                    ...
                                    thongTinTraoDoi.loaikiemtra = EnumLoaiKiemTra.CHIEUSANG;
        ...
        public RestBase Get([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                PhieuGiamSatKiemTraChieuSang? data = session.Find<PhieuGiamSatKiemTraChieuSang>(stm => stm
                    .Where($"{Sql.Entity<PhieuGiamSatKiemTraChieuSang>(x => x.id):TC} = @id")
                ...
                if (data != null)
                {
                    ...
                    data.thongTinTraoDois = session.Find<ThongTinTraoDoiKiemTra>(stm => stm
                        .Where(@$"{Sql.Entity<ThongTinTraoDoiKiemTra>(x => x.phieugiamsat_id):TC} = @id 
                            AND {Sql.Entity<ThongTinTraoDoiKiemTra>(x => x.loaikiemtra):TC} = '{EnumLoaiKiemTra.CHIEUSANG}'")
                        .WithParameters(data)
                    ).ToList();
                }
                return new RestData
        ...
        public RestBase Delete([FromRoute] int id)
        {
            using (var session = OpenSession())
            {
                var existItem = session.Get(new PhieuGiamSatKiemTraChieuSang { id = id });
                if (existItem == null)
                {
                    return new RestError
                    {
                ...
                else
                {
                    session.Execute(@$"DELETE FROM {Sql.Entity<HoSoQuanLyKiemTra>():T} 
                            WHERE {nameof(HoSoQuanLyKiemTra.phieugiamsat_id)} = @id AND {nameof(HoSoQuanLyKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.CHIEUSANG}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<AnhMinhHoaKiemTra>():T} 
                            WHERE {nameof(AnhMinhHoaKiemTra.phieugiamsat_id)} = @id AND {nameof(AnhMinhHoaKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.CHIEUSANG}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<GiaoViecNhanVien>():T} 
                            WHERE {nameof(GiaoViecNhanVien.phieugiamsat_id)} = @id AND {nameof(GiaoViecNhanVien.loaikiemtra):TC} = '{EnumLoaiKiemTra.CHIEUSANG}'", existItem);
                    session.Execute(@$"DELETE FROM {Sql.Entity<ThongTinTraoDoiKiemTra>():T} 
                            WHERE {nameof(ThongTinTraoDoiKiemTra.phieugiamsat_id)} = @id AND {nameof(ThongTinTraoDoiKiemTra.loaikiemtra):TC} = '{EnumLoaiKiemTra.CHIEUSANG}'", existItem);
        ...
        public async Task<IActionResult> Export([FromForm] KiemTraListDxDTO dto)
        {
            using (var session = OpenSession())
            {
                ...
                List<PhieuGiamSatKiemTraChieuSang> data = (await session.FindAsync<PhieuGiamSatKiemTraChieuSang>(stm => stm
                    .Where($"{condition}")
                    .WithParameters(dto)
                    .Include<PhuongThucKiemTra>(x => x.LeftOuterJoin())
                    .Include<CongCuKiemTra>(x => x.LeftOuterJoin())
                ...
                int totalCount = await session.CountAsync<PhieuGiamSatKiemTraChieuSang>(stm => stm
                           .Where($"{condition}")
                           .WithParameters(dto));

                ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
                string fileName = "BaoCaoKiemTraChieuSang.xlsx";
