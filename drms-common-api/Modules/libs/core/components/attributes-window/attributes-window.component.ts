...
import dxDataGrid, { Column, ColumnGroupCellTemplateData, SummaryGroupItem, SummaryTotalItem } from "devextreme/ui/data_grid";
...
import { EnumDataType, EnumImportFileType, EnumStatus, EnumsFunction, NO_DATA } from "../../enums/enums";
...
import "./attributes-window.component.scss";

class AttributesWindowOption {
...
class AttributesWindowComponent implements IMapComponent {
    ...
    oGISTableID: number;
    oGMap: OGMap;
    options: AttributesWindowOption;
    popup: dxPopup;
    popupTitle: string;
    ...
    constructor(oGMap, options: AttributesWindowOption) {
                        ...
                        store: new CustomStore({
                            byKey: (key, loadOptions) => {
                                return new Promise((resolve) => {
                                    TableService.record(selectedItem.lookup_table_id, key).then(data => {
                                        resolve(data);
                                    });
                                });
                            },
                            load: (options) => {
                                return new Promise((resolve) => {
                                    TableService.shortDataPaged({
                                        q: options.searchValue,
                                        skip: options.skip,
                                        table_id: selectedItem.lookup_table_id,
                                        take: options.take
                                    }).then(data => {
                                        resolve({
                                            data: data.data,
        ...
        const columnIndex: Column[] = [
            {
                ...
                cellTemplate: (container, options) => {
                    ...
                    container.append((options.rowIndex).toString());
                },
                dataField: "index",
                width: 100,
            }
        ...
        const columnsGrid: Column[] = tableColumns.map(col => {
            ...
            if (col.visible && col.column_name.includes("geom") === false) {
                if (col.lookup_table_id) {
                    let groupIndex = -1;
                    if (col.column_name.includes("matuyen")) {
                        if (self.oGISTable.table_schema === "csdl_thoatnuoc" || self.oGISTable.table_schema === "csdl_chieusang") {
                            groupIndex = 2;
                        } else {
                            groupIndex = 1;
                        }
                    } else if (col.column_name.includes("matramden")) {
                        groupIndex = 1;
                    }
                    return {
                        alignment: "left",
                        ...
                        groupCellTemplate: (container, options) => {
                            ...
                            if (options.summaryItems && options.summaryItems.length > 0) {
                                options.summaryItems.forEach(summary => {
                                    summaryText.push(summary.displayFormat
                                        .replace("{1}", summary.columnCaption || summary.name)
                                        .replace("{0}", OGUtils.formatNumber(summary.value)));
                                });
                            }
                            container.append(`${(options.displayValue || options.value)} (${summaryText.toString()})`);
                        ...
                        lookup: {
                            dataSource: {
                                ...
                                store: new CustomStore({
                                    byKey: (key, loadOptions) => {
                                        return new Promise((resolve) => {
                                            TableService.record(col.lookup_table_id, key).then(data => {
                                                resolve(data);
                                            });
                                        });
                                    },
                                    load: (options) => {
                                        return new Promise((resolve) => {
                                            TableService.shortDataPaged({
                                                q: options.searchValue,
                                                skip: options.skip,
                                                table_id: col.lookup_table_id,
                                                take: col.column_name.includes("matuyen") || col.column_name.includes("matramden") ? -1 : options.take
                                            }).then(data => {
                                                resolve({
                                                    data: data.data,
                                                    totalCount: data.recordsTotal
                    ...
                    } else if (col.column_name === "is_approved") {
                        return {
                            alignment: "center",
                            allowResizing: true,
                            allowSorting: true,
                            ...
                            //         return data[col.column_name] ? "Đã phê duyệt" : "Từ chối";
                            //     } else {
                            ...
                            calculateFilterExpression: function (filterValue, selectedFilterOperation) {
                                ...
                                return [col.column_name, "=", filterValue];
                            },
                            caption: col.name_vn,
                            // },
                            cellTemplate: function (cellElement, cellInfo) {
                                ...
                                if (cellInfo.value !== null) {
                                    text = cellInfo.value ? "Đã phê duyệt" : "Từ chối";
                                } else {
                                    text = "Chưa phê duyệt";
                                }
                            ...
                            editorOptions: {
                                dataSource: [
                                    { filterText: "Tất cả", filterValue: "" },
                                    { filterText: "Đã phê duyệt", filterValue: true },
                                    { filterText: "Từ chối", filterValue: false },
                                    { filterText: "Chưa phê duyệt", filterValue: "undefined" }
                                ],
                                displayExpr: "filterText",
                                valueExpr: "filterValue"
                            },
                            filterOperations: ["="],
                            fixed: true,
                            fixedPosition: "right",
                        ...
                        return {
                            ...
                            groupCellTemplate: (container, options) => {
                                ...
                                if (options.summaryItems && options.summaryItems.length > 0) {
                                    options.summaryItems.forEach(summary => {
                                        summaryText.push(summary.displayFormat
                                            .replace("{1}", summary.columnCaption)
                                            .replace("{0}", OGUtils.formatNumber(summary.value)));
                                    });
                                }
                                container.append(`${(options.displayValue || options.value)} (${summaryText.toString()})`);
                            ...
                            cellTemplate: (container, options) => {
                                ...
                                if (data[col.column_name]) {
                                    ...
                                    if (col.column_name.includes("ma") || OGUtils.toLowerCaseNonAccentVietnamese(col.name_vn).includes("ma")) {
                                        ...
                                        container.append(NO_DATA);
                                    }
                                }
                            },
                            dataField: col.column_name,
                            filterOperations: ["contains"],
                            groupIndex: self.oGISTable.table_schema === "csdl_thoatnuoc" && col.column_name.includes("tentuyentn") ? 1 : -1,
                            lookup: {
                                dataSource: {
                                    pageSize: 25,
                                    paginate: true,
    ...
    public for(layerInfo: OGLayerModel, tableInfo: OGTableModel, params?: { [key: string]: number | object | string; }): AttributesWindowComponent {
        const self = this;
        OGUtils.showLoading();
        self.arguments = params;

        this.oGISLayer = layerInfo;
        this.oGISTable = tableInfo || layerInfo.table;
        ...
        if (!this.oGISLayer && !this.oGISTable) {
            ...
            Promise.all([
                ...
                const groupItems = columnsResponse.filter(o => o.summary_total || o.summary_count).map(p => {
                    return {
                        column: p.column_name,
                        displayFormat: p.unit ? `{1}: {0} ${p.unit}` : "{1}: {0}",
                        name: p.name_vn,
                        summaryType: p.summary_total ? "total" : "count"
                    } as SummaryGroupItem;
                });
                if (groupItems.length === 0) {
                    groupItems.push({
                        column: this.keyColumn?.column_name,
                        displayFormat: `{0} ${this.oGISTable?.name_vn.toLowerCase()}`,
                        name: this.oGISTable?.name_vn,
                        summaryType: "count"
                    });
                }
                            ...
                            if (this.oGISTableID) {
                                ...
                                if (this.attributesGrid.option("searchPanel.text")) {
                                    ...
                                    if (!(loadOptions.filter[0] instanceof Array)) { // xử lý parse sang params bị thiếu nhiều điều kiện nếu filter phức tạp
                                        if (loadOptions.filter[0] === "province_code"
                                            || loadOptions.filter[0] === "district_code"
                                            || loadOptions.filter[0] === "commune_code"
                                        ) {
                                        ...
                                        loadOptions.filter.forEach((item, idx: number) => {
                                            if (item instanceof Array) {
                                                if (item[0] === "province_code"
                                                    || item[0] === "district_code"
                                                    || item[0] === "commune_code"
                                                ) {
                                ...
                                if (loadOptions.take) {
                                    args.take = loadOptions.take ? loadOptions.take : self.attributesGrid.pageSize();
                                }
                                ...
                                axios.post("/api/feature/advanced-search", args).then((result) => {
                                    if (result.data.status === EnumStatus.OK) {
                                        ...
                                        //         groupCount: result.data.data.dataSearch.groupCount ?? -1,
                                        //         summary: result.data.data.dataSearch.totalSummary,
                                        //         totalCount: result.data.data.dataSearch.totalCount ?? -1
                                        //     } as LoadResultObject);
                                        // } else {
                                        //     resolve({
                ...
                if (this.oGISLayer) {
                    if (this.popupTitle) {
                        this.popup.option("title", this.popupTitle);
                    }
                    else
                        this.popup.option("title", "Dữ liệu " + this.oGISLayer.name_vn);
                    this.attributesGridToolbarOptions.items.unshift({
                        ...
                        options: {
                            ...
                            items: [{
                                ...
                                onClick: () => {
                                    OGUtils.postDownload("/api/layer/export/excel", Object.assign({
                                        filter: self.attributesGrid.getCombinedFilter(),
                                        layer_id: this.oGISLayer.id
                                    }, self.arguments), "application/json");
                                },
                                text: "Xuất excel",
                            }, {
                                icon: "icon icon-export-excel",
                                onClick: () => {
                                    OGUtils.postDownload("/api/layer/export/csv", Object.assign({ layer_id: this.oGISLayer.id }, self.arguments), "application/json");
                                },
                                text: "Xuất CSV",
                            }, {
                                icon: "icon icon-export-shp",
                                onClick: () => {
                                    OGUtils.postDownload("/api/layer/export/shp", Object.assign({ layer_id: this.oGISLayer.id }, self.arguments), "application/json");
                                },
                                text: "Xuất shapefile",
                            }, {
                                icon: "icon icon-export-gdb",
                                onClick: () => {
                                    OGUtils.postDownload("/api/layer/export/gdb", Object.assign({ layer_id: this.oGISLayer.id }, self.arguments), "application/json");
                                },
                                text: "Xuất gdb",
                            }, {
                                icon: "icon icon-export-map-info",
                                onClick: () => {
                                    OGUtils.postDownload("/api/layer/export/mapinfo", Object.assign({ layer_id: this.oGISLayer.id }, self.arguments), "application/json");
                                },
                                text: "Xuất MapInfo",
                            }, {
                                template: (e) => {
                        ...
                        options: {
                            ...
                            onClick: function () {
                                self.attributesGrid.clearFilter();
                                self.attributesGrid.clearSelection();
                            ...
                            onClick: () => {
                                ...
                                if (rows && rows.length > 0) {
                                    if (this.oGISTable) {
                                        ...
                                        OGUtils.confirm("Bạn có chắc chắn phê duyệt các đối tượng này không ?", "Xác nhận").then((anws) => {
                                            if (anws) {
                                                $.ajax({
                                                    contentType: "application/json",
                                                    data: JSON.stringify(data),
    ...
    onInit(): void {
        const self = this;
        const container = $("<div id = \"popupAttributeContainer\"/>").appendTo("body");
        this.popup = container.dxPopup({
            contentTemplate: (container) => {
                container.css("padding", "0");
                this.attributesGrid = $("<div />").appendTo(container).dxDataGrid({
                    ...
                    onCellPrepared: function (e) {
                        if (e.rowType === "data") {
                            if (e.column.dataField === "is_approved") {
                                if (e.data.is_approved === null || e.data.is_approved === undefined) {
                                    e.cellElement.css("color", "#ffc107");
                                } else if (e.data.is_approved === true) {
                                    e.cellElement.css("color", "#5cb85c");
                                } else if (e.data.is_approved === false) {
                    ...
                    onSelectionChanged: () => {
                        ...
                        if (rows && rows.length === 1) {
                            ...
                            if (self.approvedButton) { self.approvedButton.option("disabled", false); }
            ...
            deferRendering: false,
            dragEnabled: true,
            height: $(document).width() > 500 ? 500 : "100%",
            hideOnOutsideClick: false,
            onHidden(e) {
                self.popupTitle = undefined;
                self.attributesGrid.clearSelection();
            },
            onResize() {
                self.attributesGrid.repaint();
            },
            resizeEnabled: true,
            shading: false,
            showCloseButton: true,
            ...
            width: $(document).width() > 900 ? 900 : "100%",
        }).dxPopup("instance");
    ...
    public setVisibleButton(showButton: boolean): this {
        this.options.showButton = showButton;
        return this;
    }
...
export { AttributesWindowComponent, AttributesWindowOption };
